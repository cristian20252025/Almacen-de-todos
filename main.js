...
function updateCartUI(){
  ...
  for(let id in cart){
    ...
    li.innerHTML = `${p.title} x${p.qty} - $${(p.price*p.qty).toFixed(2)} <button onclick="removeFromCart(${id})">🗑️</button>`;
...
function applyFilters(){
  ...
  if(sort){
    ...
    list.sort((a,b)=> dir==='asc'?a[field]>b[field]?1:-1 : a[field]<b[field]?1:-1);
  }
  renderProducts(list);
}
...
function setupCartToggle(){
  ...
  $('close-cart').addEventListener('click', ()=>$('cart').classList.add('hidden'));
}
